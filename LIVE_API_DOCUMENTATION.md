...
### Admin Patient Profile Visibility

`GET /api/v1/admin/patient_profile?family_user_id={user_id}`

Admin-only endpoint used by the web admin Users drawer to view the patient profile attached to a family user.
...
```json
{
  ...
  "data": {
    ...
    "special_instructions": "Needs walking support",
...
If no profile exists, `data` is `null` and message is `No patient profile found`.
...
| **Purpose** | Add an elder/patient profile under current family user |
...
| **Purpose** | List current family's patients |
...
{
  "booking_id": 101,
  "reason": "Caretaker cancelled last minute"
}
```
...
### Hidden/Deferred Admin Features

- Settings management APIs not implemented
